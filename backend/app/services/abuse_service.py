...
class AbuseService:
    ...
    def render_cloudflare(self, incident: Incident, brand: Brand,
        ...
        domain = domain_of(incident.target_url)
        subject = (f"Phishing report — {domain} — impersonating "
                   f"{brand.name} [Trusyn-{short_id(incident.id)}]")
        body = (
            "Cloudflare Trust & Safety,\n\n"
            f"We are reporting an active phishing site at {domain} that is\n"
            f"served behind Cloudflare and is impersonating our customer\n"
            f"{brand.name} ({brand.official_domains}).\n\n"
            f"  Reported URL:       {defang(incident.target_url)}\n"
            f"  Origin host IP:     {origin_ip or 'undisclosed (CF proxy)'}\n"
            f"  Brand impersonated: {brand.name} ({brand.official_domains})\n"
            f"  Detection:          {incident.threat_type.value if incident.threat_type else 'PHISHING'} "
            ...
            "\n"
            "Evidence collected and available on the public incident page above:\n"
            "  - Full-page screenshot of the live phishing page\n"
            "  - DOM snapshot at detection time\n"
            "  - WHOIS / RDAP record for the domain\n"
            "  - DNS A / NS / MX records at detection time\n\n"
            "Requested action (consistent with Cloudflare's published phishing\n"
            "report handling at https://developers.cloudflare.com/fundamentals/\n"
            "reference/report-abuse/):\n"
            f"  1. Display the Cloudflare phishing interstitial for {domain}.\n"
            "  2. Forward this report to the origin's hosting provider.\n"
            "  3. Notify the registrant via the WHOIS/RDAP contact.\n"
            "  4. Disclose the origin host IP to us under the Trusted Reporter\n"
            "     process so we can escalate to the upstream provider.\n\n"
            f"Please reply with the Cloudflare report ID and cite Trusyn\n"
            f"incident {incident.id} in any correspondence. We will follow up\n"
            f"in 72 hours if {domain} is still serving phishing content.\n\n"
            "Regards,\n"
            f"Trusyn Brand Protection (on behalf of {brand.name})\n"
            f"{settings.EMAILS_FROM_EMAIL}\n"
        )
